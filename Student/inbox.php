	...
	<div class="outter-wp">
                ...
                <table class="table">
					...
					$message_id = $_SESSION['message_id'];
						$query = "SELECT * FROM messages WHERE to_message_id='$message_id' AND recipientDelete='0'ORDER BY id DESC";
						$result = mysql_query($query);
						
						while($row = mysql_fetch_array($result)){
							$date = strtotime($row['time_sent']);
							$from = $row['from_message_id'];
						if($row['opened'] == 0){
							$textWeight = "msgDefualt";
						}else{
							$textWeight = "msgRead";
						}
							$query= "SELECT message_id,username FROM students WHERE message_id='$from'
							UNION
								SELECT message_id,username FROM admin WHERE message_id='$from' LIMIT 1";
							$ret =mysql_query($query) or die(mysql_error());
							while($raw = mysql_fetch_array($ret)){
								$user_id = $raw['message_id'];
								$user_name = $raw['username'];
							}
					?>
					 <tr class="table-row">
					 	 
                            <td class="table-img" valign="top">
                               <img src="student_images" alt="" />
                            </td>
                            <td  valign="top" >
                            	<?php echo $user_name; ?>
                            </td>
                            <td class="table-text"   valign="top">
                            	...
                            	<a  class="<?php echo $textWeight; ?> show" id="subj_line_<?php echo $row['id']; ?>" style="cursor: pointer; onclick="markAsRead(<?php echo $row['id']; ?>)">
                            		 <?php echo stripslashes($row['subject']); ?></a>
                               </span>
                               <div class="hidden-div"><br>
                               	<?php echo stripslashes(wordwrap(nl2br($row['message_text']),54,"\n",true));?>
                               	<br > <br ><a href="javascripttoggleReplyBox("<?php echo stripslashes($row['subject']); ?>','<?php 
                               		$sql = "SELECT * FROM students WHERE email='".$_SESSION['email']."'"; 
                               		$res = mysql_query($sql);
                               		$row = mysql_fetch_array($res);
                               			echo $row['firstname'].' '.$row['lastname'];
                               		?>','<?php echo $row['message_id']; ?>','<?php echo $user_name;?>','<?php echo $from; ?>','<?php echo @$thisRandNum; ?>)">REPLY</a>
                               	</div> 
                               	<div>                           </td>
                            <td  valign="top">
                               <input type="checkbox" class="form-control" name="<?php echo $row['id']; ?>" />
                            </td>
